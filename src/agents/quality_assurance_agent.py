...
RED_FLAG_PATTERNS = {
    "personal_pronouns": [
        r"\bI\b", r"\bme\b", r"\bmy\b", r"\bwe\b", r"\bour\b"
    ],
    "cliches": [
        "team player", "hard worker", "go-getter", "think outside the box",
        "hit the ground running", "self-starter", "results-oriented",
    ...
    "duties_not_achievements": [
        "responsible for", "duties included", "tasks involved",
        "in charge of", "handled", "worked on"
    ],
}
...
STANDARD_FONTS = [
    ...
    "Helvetica", "Verdana", "Cambria", "Garamond"
]
...
def create_quality_assurance_agent() -> Agent:
    ...
    try:
        ...
        if not agent_config:
            logger.warning(
                "quality_assurance_reviewer not found in agents.yaml, using defaults"
            )
...
def compare_experience_claims(
    ...
    try:
        ...
        for tailored_entry in tailored_exp:
            ...
            original_dates = f"{original_entry.get('start_date', '')}-{original_entry.get('end_date', '')}"
            tailored_dates = f"{tailored_entry.get('start_date', '')}-{tailored_entry.get('end_date', '')}"
...
def check_unsupported_skills(
    ...
    try:
        ...
        original_skills = set(normalize_skill(s) for s in original_skills_data)
        tailored_skills = set(normalize_skill(s) for s in tailored_skills_data)
        ...
        for skill in added_skills:
            ...
            found_in_experience = any(
                variation in original_exp_text 
...
def check_formatting_issues(tailored_resume: Dict[str, Any]) -> List[str]:
    ...
    unusual_chars = re.findall(r'[^\w\s\.\,\-\(\)\[\]\{\}\"\'\/\:\;\!\?\@\#\$\%\&\*\+\=]', resume_str)
    if len(unusual_chars) > 10:
        issues.append(f"Resume contains {len(unusual_chars)} unusual characters that may confuse ATS")
    
    # Check section headers
    sections = tailored_resume.keys()
    non_standard_sections = [
        s for s in sections 
        if s not in ["personal_info", "summary", "experience", "education", "skills", "certifications"]
...
def check_grammar_quality(tailored_resume: Dict[str, Any]) -> List[str]:
    ...
    found_cliches = [
        cliche for cliche in RED_FLAG_PATTERNS["cliches"]
        if cliche in resume_lower
    ]
    if found_cliches:
    ...
    found_duties = [
        pattern for pattern in RED_FLAG_PATTERNS["duties_not_achievements"]
        if pattern in resume_lower
    ]
    if found_duties:
        issues.append(f"Duty-focused language found: {', '.join(found_duties[:2])} (use achievements instead)")
...
def check_formatting_standards(tailored_resume: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        if actual_ordered != expected_order[:len(actual_ordered)]:
            issues.append("Sections not in standard order (Contact → Summary → Experience → Education → Skills)")
        ...
        for exp in experience:
            ...
            if bullet_count < 3:
                ...
                issues.append(f"{company}: Too many bullets ({bullet_count}). Keep to 3-6 per role.")
...
def evaluate_resume_quality_tool(
    ...
    try:
        ...
        if formatting_issues:
            ...
            overall_score -= deduction
            logger.warning(f"Deducted {deduction} points for {len(formatting_issues)} formatting issue(s)")
...
def validate_qa_output(output: Any) -> Optional[QualityReport]:
    """
    ...
    try:
        ...
        if not (0 <= quality_report.relevance.relevance_score <= 100):
            logger.warning(f"Relevance score out of range: {quality_report.relevance.relevance_score}")
            return None
...
def check_qa_quality(quality_report: QualityReport) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "issues": issues,
            "summary": (
                "QA output is complete and valid" if not issues
