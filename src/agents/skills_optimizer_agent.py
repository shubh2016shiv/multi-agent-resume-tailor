        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    log_with_context(
                        level="error",
                        message=f"LLM call failed with unexpected error",
                        function=func.__name__,
                        attempt=attempt + 1,
                        error=str(e)
                    )
...
def infer_missing_skills(
    ...
    if not missing_skills:
        log_with_context(
            level="info",
            message="No missing required skills to infer"
        )
        return []
    ...
    try:
        ...
        log_with_context(
            level="error",
            message="Skill inference JSON parsing failed",
            error=str(e)
        )
        return []
    except Exception as e:
        log_with_context(
            level="error",
            message="Skill inference failed",
            error=str(e)
        )
        return []
...
def validate_skill_inference(
    ...
    evidence_found = _verify_evidence_in_experience(
    ...
    if agent is not None:
        try:
            ...
            final_confidence = (llm_validation.get("confidence", base_confidence) * 0.6 + 
                              base_confidence * 0.4)
            log_with_context(
                level="debug",
    ...
    return SkillValidationResult(
        ...
        rejection_reason=None if final_confidence >= CONFIDENCE_THRESHOLD else "Low confidence inference"
    )
...
def _validate_with_llm(
    ...
    try:
        ...
        log_with_context(
            level="warning",
            message="LLM validation response parsing failed",
            error=str(e)
        )
...
def _verify_evidence_in_experience(
    evidence: List[str],
    experience: List[Experience]
) -> bool:
...
def _is_domain_relevant(skill: Skill, context: SkillInferenceContext) -> bool:
    ...
    experience_text = " ".join([
        exp.description.lower() for exp in context.candidate_experience
    ])
...
def prioritize_and_categorize_skills(
    ...
    log_with_context(
        level="info",
        message="Prioritizing and categorizing skills",
        total_skills=len(all_skills)
    )
    
    # Separate requirements by importance for better prompting
    must_have_reqs = [req for req in context.job_requirements.requirements if req.importance == SkillImportance.MUST_HAVE]
    should_have_reqs = [req for req in context.job_requirements.requirements if req.importance == SkillImportance.SHOULD_HAVE]
    nice_to_have_reqs = [req for req in context.job_requirements.requirements if req.importance == SkillImportance.NICE_TO_HAVE]
    
    # Determine domain-appropriate categories based on job title
    job_title_lower = context.job_requirements.job_title.lower()
    if any(term in job_title_lower for term in ['engineer', 'developer', 'software', 'ai', 'ml', 'data']):
        allowed_categories = [
...
"""
    ...
    try:
        ...
        log_with_context(
            level="error",
            message="Prioritization failed, using fallback",
            error=str(e)
        )
        return _fallback_prioritization(all_skills, context)
...
def _fallback_prioritization(
    ...
    log_with_context(
        level="info",
        message="Using enhanced fallback prioritization",
    ...
    prioritized = sorted(
        skills,
        key=lambda s: _get_skill_priority_score(s, context),
        reverse=True
    )
...
def _infer_skill_categories(
    ...
    category_patterns = {
        ...
        "AI & Machine Learning": ["langchain", "tensorflow", "pytorch", "scikit-learn", "openai", "claude", "machine learning", "ml", "ai"],
...
def optimize_skills_section(
    ...
    log_with_context(
        level="info",
        message="Skill inference complete",
    ...
    missing_inferred = [
        ...
        if skill.skill_name.lower() not in final_skill_names
    ]
...
def check_skills_quality(
    ...
    if presentation.get("skill_count", 0) < MIN_SKILL_COUNT:
        ...
        quality_report["recommendations"].append(
            "Consider adding more domain-relevant skills"
        )
    ...
    if presentation.get("skill_count", 0) > MAX_SKILL_COUNT:
        ...
        quality_report["recommendations"].append(
            "Consider removing less relevant skills"
        )
...
def _assess_keyword_coverage(
    ...
    if hasattr(job, 'ats_keywords') and job.ats_keywords:
        ...
        required_keywords = {
            req.requirement.lower() for req in job.requirements
            if req.importance == SkillImportance.MUST_HAVE
        }
