...
def get_circuit_breaker(provider: str) -> CircuitBreaker:
    ...
    if provider not in _circuit_breakers:
        ...
        failure_threshold = (
            getattr(cb_config, "circuit_breaker_failure_threshold", 5) if cb_config else 5
        )
        ...
        logger.info(
            f"Created circuit breaker for provider={provider} | "
...
def create_retry_decorator(
    ...
    return retry(
        ...
        wait=wait_exponential(multiplier=exponential_multiplier, min=initial_delay, max=max_delay),
        # Logging callbacks
    ...
    def rate_limit_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                ResilienceContext.log_event(
                    event_type="rate_limit_hit", provider=provider, limit=calls_per_minute
                )
...
def resilient_llm_call(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        _initial_delay = (
            getattr(resilience_config, "retry_initial_delay", 1.0) if resilience_config else 1.0
        )
        _max_delay = (
            getattr(resilience_config, "retry_max_delay", 60.0) if resilience_config else 60.0
        )
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                ResilienceContext.log_event(
                    event_type="llm_call_success", provider=actual_provider, function=func.__name__
                )
...
def get_resilience_stats(provider: str | None = None) -> dict[str, Any]:
    """
