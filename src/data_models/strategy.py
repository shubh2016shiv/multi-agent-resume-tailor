...
class SkillGap(BaseModel):
    ...
    suggestion: str = Field(
        ...
        examples=["Review project experience for any container orchestration work that can be highlighted as a transferable skill."]
    )
...
class AlignmentStrategy(BaseModel):
    ...
    summary_of_strategy: str = Field(
        ...
        examples=["Emphasize leadership and cloud experience, while downplaying legacy system skills. Reframe project achievements to use keywords from the job description."]
    )
    ...
    professional_summary_guidance: str = Field(
        ...
        examples=["Focus on the 5+ years of Python experience and the recent AWS certification. Mention leadership in the context of 'mentoring junior developers'."]
    )
    ...
    experience_guidance: str = Field(
        ...
        examples=["For the 'Innovate Corp' role, rewrite the first bullet point to include the keyword 'microservices'. For the 'Old Tech Co' role, condense it to two bullet points as it is less relevant."]
    )
    ...
    skills_guidance: str = Field(
        ...
        examples=["Create a 'Cloud Technologies' category and list AWS, Docker, and Kubernetes first. Remove 'Visual Basic' as it is not relevant."]
    )
