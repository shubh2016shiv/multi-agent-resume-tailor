...
def audit_bullet_structure_for_experiences(experiences: list[Experience]) -> ReviewResult:
    ...
    ####################################################
    # STEP 2: RUN THE COUNT CHECKS AND THE LENGTH CHECKS#
    ####################################################
    # We keep these as separate signals because "too few bullets" and
    # "too long bullets" are different problems with different fixes.
    comments: list[ReviewComment] = []
    comments.extend(_check_bullet_counts(experiences))
    comments.extend(_check_bullet_lengths(experiences))

    ####################################################
    # STEP 3: RETURN A SHORT SUMMARY PLUS THE DETAILED FINDINGS#
    ####################################################
    summary = (
...
def _check_bullet_counts(experiences: list[Experience]) -> list[ReviewComment]:
    ...
    ####################################################
    # STEP 1: CHECK WHETHER THE MOST RECENT ROLE IS UNDER-EXPLAINED#
    ####################################################
    # The newest role usually matters most, so too few bullets there
    # often means the strongest evidence is underdeveloped.
    findings = []
    ...
    ####################################################
    # STEP 2: CHECK WHETHER ANY ROLE HAS TOO MANY BULLETS#
    ####################################################
    # Too many bullets usually means the role needs trimming or consolidation.
...
def _check_bullet_lengths(experiences: list[Experience]) -> list[ReviewComment]:
    ...
    ####################################################
    # STEP 1: WALK THROUGH EVERY BULLET IN EVERY ROLE#
    ####################################################
    findings = []
    for role in experiences:
        for bullet_index, achievement in enumerate(role.achievements):
            ####################################################
            # STEP 2: TREAT OVER-LONG BULLETS AS PARAGRAPHS, NOT BULLETS#
            ####################################################
