...
def validate_skills_evidence(resume: Resume) -> ReviewResult:
    ...
    ####################################################
    # STEP 1: EXIT EARLY IF THE RESUME LISTS NO SKILLS#
    ####################################################
    if not resume.skills:
        return ReviewResult(comments=[], summary="No skills listed to verify")

    ####################################################
    # STEP 2: BUILD THE SKILL-LIST PLUS EVIDENCE PAYLOAD#
    ####################################################
    # We separate "skills to verify" from "evidence from the resume"
    # so the model can judge support explicitly.
...
def _build_payload(resume: Resume) -> str:
    ...
    ####################################################
    # STEP 1: NUMBER THE SKILLS SO THE MODEL CAN REFERENCE THEM CLEARLY#
    ####################################################
    skills = "\n".join(
    ...
    ####################################################
    # STEP 2: APPEND THE EVIDENCE CORPUS BUILT FROM THE REST OF THE RESUME#
    ####################################################
...
def _format_evidence(resume: Resume) -> str:
    ...
    Deliberately NOT shared with the generic resume text renderer,
    ...
    """
    ####################################################
    # STEP 1: START WITH THE NON-SKILLS SECTIONS THAT CAN SERVE AS EVIDENCE#
    ####################################################
    # We intentionally exclude the skills section because a skill cannot
    # count as evidence for itself.
    parts = [f"[Summary]\n{resume.professional_summary}"]

    ####################################################
    # STEP 2: ADD EACH NON-EMPTY EVIDENCE SECTION#
    ####################################################
    ...
    ####################################################
    # STEP 3: JOIN THE EVIDENCE SECTIONS INTO ONE PLAIN-TEXT CORPUS#
    ####################################################
    return "\n\n".join(parts)
...
def _format_experience(experiences: list[Experience]) -> str:
    ...
    ####################################################
    # STEP 1: TURN EACH ROLE INTO A SMALL EVIDENCE BLOCK#
    ####################################################
    # For this engine, skills_used matters because it is direct support
    # for listed skills even when the achievement text does not spell them out.
    blocks = []
