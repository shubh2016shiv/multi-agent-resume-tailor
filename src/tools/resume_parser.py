...
def _parse_resume_impl(file_path: str) -> str:
    ...
    try:
        ...
        logger.info(
            f"Successfully converted resume to {len(markdown_content)} characters of Markdown."
        )
        ...
        error_message = (
            f"ERROR: Failed to read or process the resume file. The file might be corrupted. {e}"
        )
        logger.error(error_message)
        return error_message
