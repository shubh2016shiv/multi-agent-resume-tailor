...
This is the lower-level function (`llm_gateway/structured_llm.py`). In plain words:
...
### `request_review(engine_id, rubric_prompt, resume_text)`

This is the convenience wrapper most judgment *review* engines use
(`llm_gateway/llm_reviewer.py`). It does two things:

1. Calls `request_structured_output(ReviewResult, rubric_prompt, resume_text)` — i.e. it
...
(regex for tabs/tables), `detect_claim_inflation` (spaCy finds names and numbers and
diffs two resumes — clever, but still pure computation).
